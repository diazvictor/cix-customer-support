...
function database:open()
	...
	if( db_type == 'pgsql' ) then
		...
		config=("user=%s password=%s dbname=%s hostaddr=%s port=%d"):format(
			conf.db.username,
			conf.db.password,
			conf.db.dbname,
			conf.db.hostname,
...
local function query(...)
	...
	elseif ( type(args[2]) == 'table' ) then
		...
		sql = (sql):format(unpack(sanitize(args[2])))
	else
		sql, err = sanitize_query(args[1])
		if not sql then
			return false, err
		...
		table.remove(args,1)
		sql = string.format(sql,unpack(sanitize(args)))
	end
	return sql
end
